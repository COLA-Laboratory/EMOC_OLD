/*
 * SPEA2_SDE.c:
 *  This file implements the main procedures of SPEA2_SDE. It is based on the following reference:
 *
...
static void SPEA2_SDE_setDistance(SMRT_individual *pop_table, int  pop_num, double **distance_arr)
{
    int i = 0, j = 0;
    SMRT_individual *temp_i = NULL, *temp_j = NULL;
    Distance_info_t *distanceInfo = NULL;
    SMRT_individual * temp_pop = NULL;
    temp_pop = (SMRT_individual *)malloc (sizeof(SMRT_individual) * pop_num);
    ...
    for (i = 0; i < pop_num; ++i)
    {
        ...
        {
            copy_individual(pop_table+index, temp_pop+index);
        }
        ...
        for(int p = 0; p < pop_num; p++)
        {
            ...
            else
            {
                ...
                {
                    if(temp_pop[i].obj[k] - temp_pop[p].obj[k] > 0.0001)
                    {
                        temp_pop[p].obj[k] = temp_pop[i].obj[k];
                    }
                    else
                    {
                        ;
...
static void SPEA2_SDE_setFitness(SMRT_individual *pop_table, int  pop_num, int para_k)
{
    ...
    SPEA2_SDE_setDistance(pop_table, pop_num, distance_arr);
...
static void SPEA2_SDE_environmentalSelect(SMRT_individual *elite_pop, SMRT_individual *offspring_pop, int para_k)
{
    ...
    {
        SPEA2_SDE_setDistance(merge_pop, merge_pop_num, distance_arr);
        //truncate operator
